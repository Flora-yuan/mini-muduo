...
int main()
{
    LOG_INFO("test_tcp_server start");

    EventLoop loop;
    TcpServer server(&loop, InetAddress(9983), "TestTcpServer");
    ...
    server.setMessageCallback([&](const TcpServer::TcpConnectionPtr& conn, Buffer* buf) {
        ...
        LOG_INFO(std::string("server received: ") + msg);
        conn->send(msg);
        conn->shutdown();
    });
    ...
    std::thread clientThread([&]() {
        ...
        serverAddr.sin_port = htons(9983);
        ...
        const std::string message = "hello tcp server";
    ...
    if (echoed) {
        LOG_INFO("test_tcp_server success");
        return 0;
    }

    LOG_ERROR("test_tcp_server failed");
    return 1;
}
